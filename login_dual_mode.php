...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // First try the vulnerable SQL injection path
    ...
    try {
        ...
        if ($result && $result->num_rows > 0) {
            // SQL injection or valid login succeeded
            ...
            // If SQL injection path fails, try normal secure login
            // This allows normal e-commerce functionality
        ...
        // On any SQL error, try the secure prepared statement method
        try {
            $stmt = $conn->prepare("SELECT * FROM users WHERE username = ? AND password = MD5(?)");
            $stmt->bind_param("ss", $user, $pass);
            $stmt->execute();
        ...
        // Catch SQL exceptions and try secure method
        try {
            $stmt = $conn->prepare("SELECT * FROM users WHERE username = ? AND password = MD5(?)");
            $stmt->bind_param("ss", $user, $pass);
            $stmt->execute();
