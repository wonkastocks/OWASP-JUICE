...
if (isset($_POST['create_user'])) {
    ...
    if ($stmt->get_result()->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Auto-assign container if available
            $stmt = $conn->prepare("SELECT port FROM containers WHERE status = 'available' ORDER BY port LIMIT 1");
            $stmt->execute();
            $result = $stmt->get_result();
            if ($row = $result->fetch_assoc()) {
                assignContainer($user_id, $row['port']);
                $message = "User created and container assigned (Port " . $row['port'] . ")";
            } else {
                $message = "User created successfully (no containers available)";
            }
...
// Handle user editing - WITH DUPLICATE CHECK
...
// Handle container assignment - WITH CHECKS
if (isset($_POST['assign_container'])) {
    ...
    if ($port > 0) {
        ...
        $stmt = $conn->prepare("SELECT port FROM containers WHERE user_id = ?");
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
        $existing = $stmt->get_result()->fetch_assoc();
        
        if ($existing) {
            $error = "User already has container on port " . $existing['port'];
        } else {
            // Check if container is available
            $stmt = $conn->prepare("SELECT status, user_id FROM containers WHERE port = ?");
            ...
            } elseif ($container['status'] !== 'available' || $container['user_id']) {
...
// Get all users with enhanced query
$conn = getDBConnection();
$users = [];
...
// Get available containers (not assigned to anyone)
$available_containers = [];
$result = $conn->query("SELECT * FROM containers WHERE user_id IS NULL AND status = 'available' ORDER BY port");
while ($row = $result->fetch_assoc()) {
...
<head>
    ...
    <style>
        ...
        .activity-active { background: #27ae60; }
        .activity-warning { background: #f39c12; }
        .activity-inactive { background: #e74c3c; }
    </style>
    <script>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php 
                                    $inactive_mins = $user['minutes_inactive'] ?? 999;
                                    if ($inactive_mins === null || $inactive_mins > 60) {
                                        echo '<span class="activity-indicator activity-inactive"></span> Inactive';
                                    } elseif ($inactive_mins > 10) {
                                        echo '<span class="activity-indicator activity-warning"></span> ' . $inactive_mins . ' min';
                                        if ($user['container_port']) {
                                            echo '<br><span class="inactive-warning">Auto-stop soon</span>';
                                        }
                                    } else {
                                        echo '<span class="activity-indicator activity-active"></span> Active';
                                    }
                                    ?>
                                </td>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="">
                ...
                <p class="alert alert-info">
                    ℹ️ User will be auto-assigned an available container if one exists
                </p>
    ...
    <div id="containerModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <select name="container_port" id="container_select" class="form-control">
                        ...
                        <option value="<?php echo $container['port']; ?>">
                            Port <?php echo $container['port']; ?> (<?php echo $container['container_name']; ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <p class="alert alert-info">
                    ℹ️ Each user can only have one container assigned at a time
                </p>
