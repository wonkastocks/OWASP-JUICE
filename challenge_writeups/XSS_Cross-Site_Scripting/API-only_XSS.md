# API-only XSS - OWASP Juice Shop Challenge Writeup

## Challenge Information

**Challenge Name:** API-only XSS
**Category:** XSS (Cross-Site Scripting)
**Difficulty:** ⭐⭐⭐ (3/6)
**Description:** Perform an XSS attack with <iframe src="javascript:alert(`xss`)">
...
This challenge demonstrates xss (cross-site scripting) vulnerabilities in a real-world web application context. Perform an XSS attack with <iframe src="javascript:alert(`xss`)">
...
#!/usr/bin/env python3
"""
Automated solver for: API-only XSS
Category: XSS (Cross-Site Scripting)
"""
...
def solve_challenge():
    """Automated solution for API-only XSS"""

    print(f"[*] Starting challenge: API-only XSS")
