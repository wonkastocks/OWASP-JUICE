...
- Internet connection to juice5.wonkatech.org
...
def solve_juice5_coupon_with_target(base_url="https://juice5.wonkatech.org"):
    ...
    if coupon_response.status_code == 200:
        ...
        if discount >= 80:
            ...
            checkout_response = session.post(checkout_url, json={})
...
def get_target_instance():
    ...
    Defaults to juice5.wonkatech.org but allows custom instances
    """
    print("\n🎯 JUICE SHOP INSTANCE SELECTION")
    print("=" * 50)
    print("Available instances:")
    print("  1. juice5.wonkatech.org (Default - Cloudflare)")
    print("  2. 66.42.93.220 (Direct native v18)")
    print("  3. 66.42.93.220:3001 (Direct v13 - Coupon compatible)")
    print("  4. Custom URL")
    print("=" * 50)

    while True:
        choice = input("\nSelect instance (1-4) [1]: ").strip()

        if choice == '' or choice == '1':
            return "https://juice5.wonkatech.org"
        elif choice == '2':
            return "http://66.42.93.220"
        elif choice == '3':
            return "http://66.42.93.220:3001"
        elif choice == '4':
